...
bool Ray::intersectsMesh(MeshCollider* mesh, OrientedPoint* instance, float& t) {
	bool intersects = false;

	auto model = instance->getModelMatrix();
	auto normalModel = instance->getNormalModelMatrix();

	for (Face& f : mesh->faces) {
		float tmp = -1.0f;
		glm::vec3 P1 = mat4vec3mult(model, mesh->points[f.i1]);
		glm::vec3 P2 = mat4vec3mult(model, mesh->points[f.i2]) - P1;
		glm::vec3 P3 = mat4vec3mult(model, mesh->points[f.i3]) - P1;
		glm::vec3 norm = normalModel * f.norm;
